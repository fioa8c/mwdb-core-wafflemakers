...
- `<YaraXRegexTab>` — top-level. Reads `sample_id` from `ObjectContext` (the same context `SamplePreview` uses). Owns regex draft + last result + in-flight state.
  ...
  - `<SampleWithHighlights>` — wraps `<ObjectPreview>` (reused from MWDB; do not reimplement language detection or syntax highlighting). Match offsets are rendered as a tinted overlay layer on top of the preview's text content.
