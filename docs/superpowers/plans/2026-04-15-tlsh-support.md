...
**Architecture:** One new pypi dependency (`py-tlsh==4.12.1`), one new thin wrapper module (`mwdb/core/tlsh.py`) mirroring the structure of `mwdb/core/ssdeep.py`, one new nullable indexed `String(72)` column on the `object` table (physical storage — `File`/`Config`/`TextBlob` share it via single-table inheritance since migration `6db157d09a30`; `File.tlsh` class attribute resolves to `object.tlsh`) via Alembic migration, and additive-only edits to the schema/search/frontend layers that already expose ssdeep. No config flag (TLSH is always on), no backfill of existing rows, no similar-files endpoint in this subsystem.
...
- `mwdb/model/migrations/versions/<new>_add_tlsh_column_to_file.py` — Alembic migration that adds the `tlsh` column to the `object` table and creates the `ix_object_tlsh` index. `down_revision = "01dea56ffaf7"`. (Filename reflects the SQLAlchemy-model intent; the physical target is `object` due to polymorphism.)
...
(Use `flask db revision` — plain skeleton without autogenerate — because we want an explicit hand-written migration that exactly matches the spec. Autogenerate would work, but the spec requires a specific index name (`ix_object_tlsh`) and the exact column declaration, and hand-writing is more reliable than editing autogenerated output.)
...
```bash
docker compose -f docker-compose-dev.yml exec postgres psql -U mwdb -d mwdb -c "\d object" | grep -E "tlsh|ix_object_tlsh"
```

Expected: one line showing `tlsh | character varying(72) | ... |` (collation shown separately) and an index line `"ix_object_tlsh" btree (tlsh)`.
...
Nullable String(72) column on the 'object' table with ix_object_tlsh btree index. Existing
...
Exact-match only, hits the ix_object_tlsh index. No Luqum operator
